...
namespace Cmsbox\Paybox\Gateway\Processor;
...
class Connector
{
    ...
    {
        $mode = $config->params[\Cmsbox\Paybox\Gateway\Config\Core::moduleId()][self::KEY_ENVIRONMENT];
    ...
    public static function getSecretKey($config)
    {
        ...
        switch ($config->base[self::KEY_ENVIRONMENT]) {
            case 'simu':
                $key = $config->params[\Cmsbox\Paybox\Gateway\Config\Core::moduleId()][self::KEY_SIMU_SECRET_KEY];
                break;

            case 'test':
                $key = $config->params[\Cmsbox\Paybox\Gateway\Config\Core::moduleId()][self::KEY_TEST_SECRET_KEY];
                break;

            case 'prod':
                $key = $config->params[\Cmsbox\Paybox\Gateway\Config\Core::moduleId()][self::KEY_PROD_SECRET_KEY];
                break;
        }
